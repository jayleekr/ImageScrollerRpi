...
volatile static EGLDisplay GDisplay;
volatile static  EGLSurface GSurface;
volatile  static EGLContext GContext;

void InitGraphics(int32_t layer)
{	bcm_host_init();
	int32_t success = 0;
	EGLBoolean result;
	EGLint num_config;
...
void updateScreen() {
	...
	//fprintf(stderr, "GDisplay : %d  GSurface: %d\n",GDisplay ,GSurface );
 	eglSwapBuffers(GDisplay,GSurface);
 //fprintf(stderr, "eglSwapBuffers : %d\n", eglSwapBuffers(GDisplay,GSurface) );
 	//check();
}
